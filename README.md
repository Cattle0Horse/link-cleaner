...
如果某个网站不适合自动清洗，可以在脚本菜单中选择“在当前网站禁用清洗”。禁用按当前页面完整 `hostname` 生效，例如在 `example.com` 禁用清洗，不会影响脚本在 `www.example.com` 或其它子域名上运行。禁用后，脚本只是在当前网站不执行清洗规则，会跳过当前页面 URL 自动清洗、网页链接自动清洗、动态新增链接清洗、SPA 路由变化后的清洗，以及增强清洗模式中的 xhr/fetch 请求清洗；如果你在其它网站遇到指向该网站的链接，相关清洗规则仍会照常执行。手动输入链接清洗和复制标题/网址功能仍可使用。需要恢复时，在同一网站的脚本菜单中选择“重新启用当前网站清洗”。切换后刷新页面生效。
