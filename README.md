...
脚本菜单支持按当前网站切换是否执行自动清洗逻辑。默认使用白名单模式：所有网站都不会自动清洗，只有在脚本菜单中选择“启用当前网站清洗”的网站才会执行清洗。也可以切换为黑名单模式：所有网站都会自动清洗，可以在脚本菜单中选择“禁用当前网站清洗”把当前网站加入黑名单。两套名单互相独立，切换模式不会清空另一套名单；切换模式、加入或移除当前网站后刷新页面生效。

当前网站设置按当前页面完整 `hostname` 生效，例如在 `example.com` 禁用或启用清洗，不会影响脚本在 `www.example.com` 或其它子域名上运行。当前网站不执行清洗时，脚本会跳过当前页面 URL 自动清洗、网页链接自动清洗、动态新增链接清洗和 SPA 路由变化后的清洗；如果你在其它网站遇到指向该网站的链接，相关清洗规则仍会照常执行。手动输入链接清洗和复制标题/网址功能仍可使用。这里的黑名单/白名单指当前网站是否执行清洗逻辑，不是 `cleanFactory.blacklist` / `cleanFactory.whitelist` 中的链接参数清洗规则。
